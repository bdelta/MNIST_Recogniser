...
class NNModel(object):
    ...
    def __init__ (self, weight_path=None):
        ...
        # Tensorflow weights
        self.input_weights = tf.Variable(tf.random_normal([in_size, hidden]))
        self.input_biases = tf.Variable(tf.random_normal([hidden]))
        self.hidden_weights = tf.Variable(tf.random_normal([hidden, output]))
        self.hidden_biases = tf.Variable(tf.random_normal([output]))
    ...
    def feedforward(self, data):
        ...
        z_3 = tf.matmul(a_2, self.hidden_weights) + self.hidden_biases
        return z_3
